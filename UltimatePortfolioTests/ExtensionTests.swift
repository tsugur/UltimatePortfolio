...
final class ExtensionTests: XCTestCase {
	func testKeyPathSequenceSortingSelf() {
		let items = [1, 4, 3, 2, 5]
	...
	func testSequenceKeyPathSortingCustom() {
		...
		let sortedItems = array.sorted(by: \.value, using: >)
	...
	func testDecodingString() {
		...
		XCTAssertEqual(data, "The rain in Spain falls mainly on the Spaniards.", "The string must match DecodableString.json")
	}
